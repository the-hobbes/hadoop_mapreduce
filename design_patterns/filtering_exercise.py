...
def mapper():
    ...
    for line in reader:
        ...
        if not re.search('[!?.]', body):
            # there are no punctuation characters in the body, and the counter is incremented
            running_count += 1
            writer.writerow(line) # the writerow stuff is to pass udacity's assertions
        else:
            # there are punctuation characters somewhere. Is there one at the end of the body?
            # Are there punctuation characters elsewhere besides the end of the body?
            if body.endswith(punctuation_chars) and not re.search('[!?.]', body[0:-1]):
                running_count += 1
